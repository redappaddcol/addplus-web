...
export default async function PanelAliadoPage() {
  ...
  if (!perfil || (perfil.rol !== "aliado" && perfil.rol !== "admin")) {
    redirect("/");
  }
  ...
  const { data: causas } = await supabase
    .from("causes")
    .select(
      "id, titulo, descripcion, ciudad, tiene_meta_economica, voluntarios_requeridos, categories(nombre)"
    )
    .eq("estado", "activa")
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-neutral-600">
        Hola {perfil.nombre}. Estas son las causas activas donde tu organización puede aportar
        recursos, conocimiento o convocatoria — de forma visible con tu marca, o invisible si tu
        interés es puramente social.
      </p>
          ...
          causas.map((causa: any) => (
            <div key={causa.id} className="rounded-lg border border-neutral-200 bg-white p-5">
              ...
              <p className="text-sm text-neutral-500">
                {causa.ciudad || "Sin ciudad"} · {causa.voluntarios_requeridos} voluntarios requeridos
                {causa.tiene_meta_economica ? " · con meta económica" : ""}
              </p>
